...
# Global to be used for updates. Should be cleared after each file has been
# completed.
seq = SeqGenerator()
...
class MRTExtractor:
    ...
    def __init__(self, input):
        ...
        seq = SeqGenerator()    # Reset seq for each file
...
class RIBExtractor(MRTParser):
    ...
    def get_line(self, prefix, next_hop):
        """ Get a line of data for the RIB table. NOTE that the snapshot field
        must be appended separately.
        """
        return (prefix, self.peer_as, self.peer_ip, int(self.ts) * 1000, self.merge_as_path())
